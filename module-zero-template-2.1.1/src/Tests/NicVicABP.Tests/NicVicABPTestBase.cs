...
namespace NicVicABP.Tests
{
    public abstract class NicVicABPTestBase : AbpIntegratedTestBase<NicVicABPTestModule>
    {
        ...
        protected void UsingDbContext(Action<NicVicABPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<NicVicABPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<NicVicABPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<NicVicABPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<NicVicABPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NicVicABPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<NicVicABPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NicVicABPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<NicVicABPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<NicVicABPDbContext, Task<T>> func)
        {
            T result;
